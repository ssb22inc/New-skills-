...
- target: `phase`
- tree: `860de6b6ac8ceab3cbd682910782a2317555d770`
- branch: `claude/fullburn-engine-spec-r7v5lg`
- started: 2026-09-20T23:49:27.473Z
...
| ↳ D-adversary | deliverable: CLAUDE.md + adversary agent installed and discoverable | **PASS** | `vitest run engine/test/invariants/invariants.test.ts -t discovery mirror` | 3 passed, 0 failed (34) |
| ↳ D-ci | deliverable: CI pipeline — every workflow SHA-pinned, permission-declared, scope-filtered inside the job | **PASS** | `vitest run engine/test/invariants/invariants.test.ts -t workflow hygiene` | 3 passed, 0 failed (34) |
...
| C2 | §2.1.2 same-family adversary round PASS against this tree | **FAIL** | `checkAdversaryReport(phase 0, 14 report(s), tree 860de6b6ac8c)` | ADVERSARY_REPORT_phase0.r9.md: report has no readable 'verified-tree:' binding in its first 10 lines at column 0 — it cannot be shown to be about a different tree, so it blocks (fail closed) (unresolved business on this tree blocks regardless of any PASS) |
| C3 | §2.1.3 cross-family read PASS against the SAME tree, artifact committed | **FAIL** | `checkAdversaryReport(non-Claude reports only, tree 860de6b6ac8c)` | no report under reports/ carries a `Reviewer-family:` line naming a non-Claude family (14 report(s) read) |
...
| C5 | §2.1.5 mutation harness: 0 survived, 0 stale, meta-check passed in the same run | **PASS** | `node engine/scripts/mutate.mjs` | meta-check ok; 231 mutations: 231 caught, 0 survived, 0 stale |
| C6 | §2.1.6 guard population enumerated from source, one-to-one coverage, every guard driven | **PASS** | `vitest run engine/test/invariants -t 'every money-path guard is still reachable'` | 1 passed, 0 failed (34); the 'disabled individually and caught' half is C5's per-guard entries |
| C7 | §2.1.7 suite green under ≥5 seeds; typecheck, lint, leak-check clean; SIGINT drill | **FAIL** | — | 8/9 sub-conditions passing |
| ↳ C7-seed-7 | full suite, shuffled seed 7 | **PASS** | `vitest run --sequence.shuffle --sequence.seed=7` | 425 passed, 0 failed (425) |
| ↳ C7-seed-42 | full suite, shuffled seed 42 | **PASS** | `vitest run --sequence.shuffle --sequence.seed=42` | 425 passed, 0 failed (425) |
| ↳ C7-seed-1234 | full suite, shuffled seed 1234 | **PASS** | `vitest run --sequence.shuffle --sequence.seed=1234` | 425 passed, 0 failed (425) |
| ↳ C7-seed-2026 | full suite, shuffled seed 2026 | **PASS** | `vitest run --sequence.shuffle --sequence.seed=2026` | 425 passed, 0 failed (425) |
| ↳ C7-seed-9001 | full suite, shuffled seed 9001 | **PASS** | `vitest run --sequence.shuffle --sequence.seed=9001` | 425 passed, 0 failed (425) |
...
| C9 | §2.1.9 every ledger/CLAUDE.md behavioural row carries a test that fails when stale | **PASS** | `vitest run engine/test/invariants -t 'every behavioural claim in the ledger still holds'` | 1 passed, 0 failed (34) |
...
**INCOMPLETE — 15 failing:** C1, AC1-live, AC4-enforced, D-vault-rotation, D-warehouse, D-name, C2, C3, C4, C7, C7-lint, C8, C8-owed, C8-identity, C10
