import { capsOf, fixedCaps, memoryMeter, testClock } from "./helpers.ts";
...
describe("spend meter — reserve/settle (F1, F2, F3)", () => {
  it("reservations accumulate against the cap before any of them settle", () => {
    const m = memoryMeter(testClock, capsOf(0.05, 0.05));
  ...
  it("non-finite accounting refuses spend instead of sliding through a NaN comparison (F2)", () => {
    ...
    const noCaps = memoryMeter(testClock, (() => undefined) as never);
    expect(() => noCaps.reserve("c", 0.01)).toThrow(MeterUnavailableError);
    const badZone = memoryMeter(testClock, () => ({ dailyUsd: 5, monthlyUsd: 5, timeZone: "Mars/Olympus" }));
  ...
  it("an unavailable meter refuses everything (fail closed)", () => {
    ...
    const ledger = new InMemorySpendLedger(testClock, capsOf(0.05, 0.05));
    const m = new MemorySpendMeter(ledger);
