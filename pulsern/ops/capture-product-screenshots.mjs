...
async function screenshot(page, name, expectedText) {
  ...
  for (const text of expectedText) {
    if (!bodyText.includes(text)) throw new Error(`${name} is missing expected product text: ${text}`);
  }
...
export async function captureProductScreenshots() {
  ...
  const server = await createServer({
    ...
    plugins: [{
      ...
      resolveId(source, importer) {
        if (source === "./supabase.js" && /\/src\/[^/]+$/.test(importer ?? "")) {
  ...
  try {
    ...
    await page.getByRole("button", { name: "Start today's round", exact: true }).waitFor();
