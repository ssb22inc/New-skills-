...
export const COMMERCIAL_VERIFIED_AT = "2026-09-08";
const SITE = "https://www.pulsern.app";
import { LIBRARY } from "../src/library.js";
const QS = LIBRARY.questions.published;   // 10,034 — every plan, no tiering
const CASES = LIBRARY.cases.published;
const CARDS = LIBRARY.cards.published;
...
const pages = [
  {
    ...
    body: `<p class="lead">The useful question is not “Which company says it is best?” It is “Which study system matches the way I need to prepare?” PulseRN, UWorld, Archer Review, and Kaplan overlap in practice questions and exam preparation, but their public packages emphasize different kinds of support. On publicly listed volume PulseRN is the largest of the four, at ${QS} questions on every plan; on breadth of media, live teaching and course structure it is deliberately the narrowest.</p>
      ${disclosure}
      ...
      <section><h2>PulseRN’s fit in one paragraph</h2><div class="card"><p>PulseRN lists ${QS} practice questions, ${CASES} case studies and ${CARDS} study cards on every paid plan, and is built and clinically owned by a working registered nurse. Inside one study app, learners can practice adaptively, revisit missed questions, use spaced-repetition cards, open a searchable lab-value reference, ask educational NCLEX or nursing-concept questions, and request a different AI explanation after answering. AI assists the study experience but is not presented as a clinician, a human reviewer, or a pass predictor.</p></div></section>
  ...
  },
  {
    ...
    claims: [
      ...
      { id: "pulse-bank-scale", statement: `PulseRN publicly lists ${QS} practice questions on every paid plan, against UWorld's publicly listed 3,400+ at its longest duration.`, sourceIds: ["pulsePricing", "pulseHome", "uworldCourse"] },
    ],
    body: `<p class="lead"><strong>${QS} practice questions. $99. Free for a day, no credit card.</strong> That is what PulseRN includes on every plan, alongside ${CASES} case studies, ${CARDS} study cards, adaptive practice, missed-question repair and a searchable lab and vital-sign reference in the same app. UWorld's public course page lists up to 3,400+ questions at its longest duration for a $149 list price.</p>
      <div class="card best"><span class="tag">Try it before you pay</span><h2 style="margin-top:0">One day of full study access, no card required</h2><p>You do not have to take any of this on trust. Open the whole study library for a day without entering payment details, and judge the workflow before spending anything.</p><p><a class="button" href="/?start=1">Start PulseRN free for one day</a><a class="button alt" href="/pricing/">See transparent pricing</a></p></div>
      <section><h2>Who is accountable for ${QS} questions</h2><div class="card"><p>Volume only counts if the content is sound, so here is how it is made. PulseRN is created and clinically owned by <a href="/about/#sheldon-bennett-rn">Sheldon Bennett, RN</a>, whose licence can be verified against the state register. Every practice item passes cross-vendor adversarial review before publication: one model writes it, a model from a different vendor attacks the keyed answer, checks currency of practice and hunts for a second defensible option, and items that fail are discarded rather than published. Reviewer notes are kept on every item and the review console retains rejection authority. AI assists the study experience; it is not presented as a clinician, a human reviewer, or a pass predictor.</p></div></section>
      ${disclosure}
      ${table("PulseRN and UWorld public-feature comparison", [
        ["Practice questions", `${QS} on every paid plan`, "Publicly lists up to 3,400+ by duration; the 90-day tier lists 3,100+", `PulseRN publicly lists roughly three times UWorld's largest advertised total, and does not tier content by plan length. Counts are provider-defined and do not by themselves prove question quality.`],
        ["Public 30-day price", "$99, after a free day that needs no card", "$149 list price; a $139 bundle promotion was displayed when checked", "UWorld listed $50 more at the observed list prices. Verify current checkout pricing on both sites."],
  ...
  },
  {
    ...
    body: `<p class="lead"><strong>${QS} practice questions. A free day, no credit card.</strong> That is what PulseRN includes on every plan at $99, with ${CASES} case studies, ${CARDS} study cards, a searchable lab and vital-sign reference and educational nursing Q&amp;A in the same app. Archer's public package page lists 3,100+ questions at the same $99 one-month list price, currently shown discounted to $79.</p>
      <div class="card best"><span class="tag">Try it before you pay</span><h2 style="margin-top:0">Same list price. Roughly three times the questions.</h2><p>Both products list a one-month plan at $99. Archer was showing a $79 promotion when we checked, and promotions move, so compare the checkout pages on the day you buy. What does not move is what the money buys: PulseRN lists ${QS} practice questions to Archer's 3,100+. You can confirm that for yourself in a day of full access, without entering payment details.</p><p><a class="button" href="/?start=1">Start PulseRN free for one day</a><a class="button alt" href="/how-it-works/">See how PulseRN works</a></p></div>
      <section><h2>Who is accountable for ${QS} questions</h2><div class="card"><p>A larger bank is only an advantage if the items hold up, so here is how they are made. PulseRN is created and clinically owned by <a href="/about/#sheldon-bennett-rn">Sheldon Bennett, RN</a>, whose licence can be verified against the state register. Every practice item passes cross-vendor adversarial review before publication: one model writes it, a model from a different vendor attacks the keyed answer, checks currency of practice and hunts for a second defensible option, and failures are discarded rather than published. Reviewer notes are retained on every item and the review console keeps rejection authority. AI assists the study experience; it is not presented as a clinician, a human reviewer, or a pass predictor.</p></div></section>
      ${disclosure}
      ${table("PulseRN and Archer Review public-feature comparison", [
        ["Practice questions", `${QS} on every paid plan`, "Public package page lists 3,100+ questions", `PulseRN publicly lists roughly three times Archer's advertised total. Counts are provider-defined; also weigh rationales, workflow, and unused-question rules.`],
        ["One-month list price", "$99 for 30 days, after a free day that needs no card", "$99 list for Q-Bank + CAT, shown discounted to $79 when checked", `Both list at $99, so the like-for-like question is what the same money buys: ${QS} practice questions versus 3,100+. Archer's discount may change; verify both checkout pages.`],
        ["What $99 buys", `${QS} questions, ${CASES} case studies, ${CARDS} study cards`, "3,100+ questions, 30+ readiness and CAT exams", "At the same list price PulseRN lists roughly three times the practice volume; Archer lists far more formal assessment forms. Weigh which you will actually work through."],
  ...
  },
  {
    ...
    body: `<p class="lead">Kaplan’s public NCLEX offering spans question-bank practice, CATs, an AI tutor, strategy instruction, content review, live classes, tutoring, and a conditional guarantee. PulseRN is a narrower self-directed product designed around one integrated app and a directly accountable working-RN creator.</p>
      ...
      ${table("PulseRN and Kaplan public-feature comparison", [
        ...
        ["Question practice", `${QS} on every paid plan`, "Public pages list a QBank and larger course inventories, with quantities varying by package presentation", "Package definitions differ, so a single count is not an apples-to-apples quality measure."],
  ...
  },
  {
    ...
    body: `<p class="lead">There is no defensible universal first-place QBank. The useful shortlist depends on whether you need visual teaching, high-volume readiness practice, formal strategy instruction, or an integrated RN-owned app with lab lookup and broad nursing-study Q&amp;A.</p>
      ...
      <section><h2>Question counts need context</h2><p>PulseRN lists ${QS} practice questions on every paid plan, with no tiering by access length. UWorld lists totals that grow from 2,900+ to 3,400+ by duration. Archer lists 3,100+ in its RN QBank packages. Kaplan’s public pages describe a QBank and larger course inventories, while quantities can vary by package presentation. These are provider-defined totals: some may include assessment questions, course activities, or access-specific inventory. Do not treat the largest number as audited proof of the best bank.</p></section>
    ...
    faq: [
      { q: "Which NCLEX question bank has the most questions?", a: `Of the four compared here, PulseRN lists the largest public total: ${QS} on every paid plan, against up to 3,400+ for UWorld and 3,100+ for Archer, while Kaplan lists different totals by practice or course package. Totals are provider-defined and may count different things, so treat volume as one factor rather than proof of quality.` },
  ...
  },
  {
    ...
    body: `<p class="lead">If you are here after a first attempt, the fear is not overpaying &mdash; it is sitting the exam again and getting the same result. What changes a second attempt is not more of the same questions, but finding what actually failed, repairing the reasoning behind it, and practising on enough material you have genuinely not seen before. <strong>PulseRN lists ${QS} practice questions on every plan</strong>, which is roughly three times either of the large QBanks, plus missed-question review, adaptive sequencing and readiness forms that never repeat an item. No product can promise you a different outcome, and this page does not.</p>
      ${disclosure}
