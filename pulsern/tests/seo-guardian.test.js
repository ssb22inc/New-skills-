...
import { articleJsonLd, related } from "../ops/build-learn.mjs";
import { ARTICLES } from "../ops/learn-content.mjs";
...
describe("SEO guardian", () => {
  ...
  it("distributes contextual related links across every guide", () => {
    const inbound = new Map(ARTICLES.map((article) => [article.slug, 0]));
    for (const article of ARTICLES) {
      const recommendations = related(article, ARTICLES);
      expect(recommendations).toHaveLength(3);
      expect(new Set(recommendations.map((item) => item.slug)).size).toBe(3);
      expect(recommendations.some((item) => item.topic === article.topic)).toBe(true);
      expect(recommendations.some((item) => item.slug === article.slug)).toBe(false);
      for (const recommendation of recommendations) inbound.set(recommendation.slug, inbound.get(recommendation.slug) + 1);
    }
    expect(Math.min(...inbound.values())).toBeGreaterThanOrEqual(1);
  });

  it("publishes one truthful free-practice hub over all four reviewed sample sets", async () => {
    const html = await fs.readFile(new URL("../public/free-nclex-practice-test/index.html", import.meta.url), "utf8");
    expect(auditHtml("/free-nclex-practice-test/", html).findings).toEqual([]);
