  ...
  describe('GET /api/listings', () => {
    ...
    it('filters by city', async () => {
      ...
      data.listings.forEach((listing: any) => {
        expect(listing.city.toLowerCase()).toContain('houston');
      });
    });
    ...
    it('filters by price range', async () => {
      ...
      expect(response.status).toBe(200);
      data.listings.forEach((listing: any) => {
        expect(listing.price_monthly).toBeGreaterThanOrEqual(1500);
        expect(listing.price_monthly).toBeLessThanOrEqual(2500);
      });
    });

    it('sorts by price ascending', async () => {
      const response = await fetch('/api/listings?sortBy=price_asc');
      const data = await response.json();

      expect(response.status).toBe(200);
      for (let i = 1; i < data.listings.length; i++) {
        expect(data.listings[i].price_monthly).toBeGreaterThanOrEqual(
          data.listings[i - 1].price_monthly
        );
      }
    });
  });
  ...
  describe('POST /api/listings', () => {
    it('creates a new listing', async () => {
      const newListing = {
        ...
        description: 'A detailed description of the property that is at least 50 characters long for validation.',
