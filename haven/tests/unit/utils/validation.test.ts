...
describe('listingSchema', () => {
  const validListing = {
    ...
    description:
      'A wonderful apartment with great views and modern amenities. Perfect for travel nurses looking for a comfortable stay near the hospital. Fully furnished with all necessities.',
  ...
  it('rejects invalid zip codes', () => {
    expect(() => listingSchema.parse({ ...validListing, address_zip: '123' })).toThrow();
  });

  it('rejects prices out of range', () => {
    expect(() => listingSchema.parse({ ...validListing, price: 50 })).toThrow();
    expect(() => listingSchema.parse({ ...validListing, price: 200000 })).toThrow();
  });
});
