  ...
  describe('calculateMatchScore', () => {
    it('calculates total score from weighted components', async () => {
      const seeker = createMockSeekerProfile({
        ...
        must_haves: ['wifi'],
        lifestyle: { work_from_home: false, cleanliness: 8, noise_tolerance: 5, sleep_schedule: 'night_owl', guest_frequency: 'rarely' },
      });
      ...
      const result = await calculateMatchScore(seeker as any, listing as any);
    ...
    it('calculates budget score correctly when within budget', async () => {
      ...
      expect(result.scores.budget).toBeGreaterThanOrEqual(70);
      expect(result.breakdown.budget.percent_of_max_budget).toBeLessThanOrEqual(100);
    });
    ...
    it('penalizes over-budget listings', async () => {
      ...
      expect(result.scores.budget).toBeLessThan(70);
    });
    ...
    it('uses custom weights when provided', async () => {
      ...
      const result = await calculateMatchScore(seeker as any, listing as any, {
        weights: {
          location: 0.5, // Heavily weight location
          budget: 0.3,
          lifestyle: 0.1,
          personality: 0.05,
          amenity: 0.05,
