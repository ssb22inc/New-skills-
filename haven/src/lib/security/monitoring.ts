...
class SecurityMonitor {
  ...
  async trackEvent(
    ...
    // Clean old events
    const threshold =
      this.alertThresholds[eventType as keyof typeof this.alertThresholds];
    if (threshold) {
  ...
  async triggerAlert(alert: SecurityAlert): Promise<void> {
    ...
    try {
      ...
      console.error('[Security Monitor] Failed to store alert:', error);
    }
  ...
  private async sendAlertNotification(alert: SecurityAlert): Promise<void> {
    ...
    if (process.env.SLACK_SECURITY_WEBHOOK) {
      try {
        ...
        console.error('[Security Monitor] Failed to send Slack notification:', error);
      }
    }
  }
  ...
  private async autoRemediate(alert: SecurityAlert): Promise<void> {
    ...
    try {
      ...
      console.error('[Security Monitor] Auto-remediation failed:', error);
    }
  }
